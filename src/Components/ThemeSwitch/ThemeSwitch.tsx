...
const ThemeSwitch = () => {
    ...
    const [logoImage, setLogoImage] = useState('/src/assets/images/logo.svg');
    const handleLogoChange = (event: React.FormEvent<HTMLInputElement>) => {
        setLogoImage(event.currentTarget.value);
        dispatch(updateLogoImage(logoImage));
    }
    return (
        <>
            <div>
            ...
            <dialog id="customThemeContainer" className="customThemeContainer">
                ...
                <div>
                    <input type="text" id="head1" name="head1" value={logoImage} onChange={handleLogoChange} />
                    <label htmlFor="head1">Logo image</label>
                </div>
